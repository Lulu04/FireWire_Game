...
type
  ...
  public
   ...
   function AddPoint( aXPercent, aYPercent: single; IsJump: boolean; aSelectIt: boolean ): integer; // [0..1] range
...
begin
 ...
 if ( Button = mbLeft ) and ( IndexOverMouse = -1 )
   then begin
         flag:=TRUE;
         i := AddPoint( PaintBoxXToPercentX(X), PaintBoxYToPercentY(Y), FALSE, TRUE);
         FCurveChanged := TRUE;
...
begin
 ...
 if FPtDraggedByMouse <> NIL
   then begin // mouse drag point
         ...
         // update point coord.
         FPtDraggedByMouse^.Point.x := PaintBoxXToPercentX(X);
         FPtDraggedByMouse^.Point.Y := PaintBoxYToPercentY(Y);
         FCurveChanged := TRUE;
         PB.Invalidate ;
        end
  ...
  begin
   ...
   if FList.Count>0 then begin
     ...
     for i:=1 to FList.Count-1 do
      begin
       x1 := PercentXToPaintBoxX( p1^.Point.x );
       y1 := PercentYToPaintBoxY( p1^.Point.y );
       p2 := GetPointByIndex(i);
       x2 := PercentXToPaintBoxX( p2^.Point.x );
       y2 := PercentYToPaintBoxY( p2^.Point.y );
       if not p2^.IsJump then DrawLineAntialias(x1,y1,x2,y2, FLineColor, 1.5 );
       p1 := p2
      end;
...
var p: TPointF;
begin
 p.x := PercentXToPaintBoxX(APts^.Point.x);
 p.y := PercentYToPaintBoxY(APts^.Point.y);
...
function TGraphEditor.AddPoint(aXPercent, aYPercent: single; IsJump: boolean; aSelectIt: boolean): integer;
var o: PGraphPoint;
begin
 ...
 new( o );
 o^.Point := PointF( aXPercent, aYPercent );
 o^.IsJump := IsJump;
...
begin
 ...
 try
   ...
   for i:=0 to FList.Count-1 do begin
    ...
    txt+=FormatFloat('0.000', p^.Point.x)+' '+FormatFloat('0.000', p^.Point.y);
    s.Add(txt);
   end;
    ...
    then Exception.Create('File '+ExtractFileName(aFilename)+' List of points not found...')
      ...
      repeat
        ...
        if Length( SplittedTxt )=2
          then AddPoint( StrToFloat(SplittedTxt[0]), StrToFloat(SplittedTxt[1]), FALSE, FALSE )
          else if Length( SplittedTxt )=3
            then AddPoint( StrToFloat(SplittedTxt[1]), StrToFloat(SplittedTxt[2]), TRUE, FALSE )
...
begin
 for i:=0 to FList.Count-1 do
  with GetPointByIndex(i)^ do begin
   ...
   p.x := PercentXToPaintBoxX(Point.x) + aDeltaX;
   if p.x>PB.Width-1
     then p.x := p.x-PB.Width-1
   else if p.x<0 then p.x := p.x+PB.Width-1;

   p.y := PercentYToPaintBoxY(Point.y) + aDeltaY;
   if p.y>PB.Height-1
     then p.y := p.y-PB.Height-1
   else if p.y<0 then p.y := p.y+PB.Height-1;

   Point.x := PaintBoxXToPercentX(p.x);
   Point.y := PaintBoxYToPercentY(p.y);
  end;
 PB.Invalidate;
end;
